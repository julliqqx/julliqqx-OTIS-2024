...
public:
    // Конструктор принимает количество временных шагов и желаемую температуру
    NonlinearModel(int timeSteps, double desiredTemperature = 10)
        : time(timeSteps), desiredTemp(desiredTemperature) {}

    // Метод для расчета модели
    void calculateModel() {
        for (int i = 0; i < time; i++) {
            // Обновление параметра с использованием синуса
            param[3] = std::sin(param[3]);

            // Рассчитываем новое значение futureY
            double futureY = calculateFutureY();
