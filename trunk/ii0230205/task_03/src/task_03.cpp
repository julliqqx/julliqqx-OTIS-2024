...
public:
    ...
    vector<int> findEulerianCycle() const {
        if (!hasEulerianCycle()) return {}; // Убраны фигурные скобки
    ...
    vector<int> findHamiltonianCycle() const {
        ...
        visited[0] = true;
        if (hamiltonianDFS(0, path, visited)) return path; // Убраны фигурные скобки
        return {};
    }
        ...
        while (!q.empty()) {
            ...
            for (int neighbor : adjList[curr]) {
                if (!visited[neighbor]) {
                    ...
                    visited[neighbor] = true; // Перенесено в блок if
                    q.push(neighbor);
                }
            }
        }
    ...
    bool hamiltonianDFS(int node, vector<int>& path, vector<bool>& visited) const {
        ...
        for (int neighbor : adjList[node]) {
            if (!visited[neighbor]) {
                ...
                if (hamiltonianDFS(neighbor, path, visited)) return true; // Убраны фигурные скобки
                path.pop_back();
